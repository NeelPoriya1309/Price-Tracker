...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const pgResponse = await client.query(`update public.location set ${updatedLocation} where id='${id}'`)
    ...
    try {
      const response = await client.query(`delete from public.location where id='${id}'`)
